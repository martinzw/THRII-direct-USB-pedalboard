...
static volatile int8_t last_solo_id;       //highest (0-based) id for solo in the last group (-1 if no dedicated solo file in last patch) (relative number)
...
void setup()  //do preparations
{
	...
	//e.g.: 15 patches => last_settings_id = 4
	//e.g.:  0 patches =>                   -1
	//e.g.:  1 patch   =>                    0
	//e.g.: 20 patches =>                    4

	max_group_id=(npatches!=0)?npatches/10:-1; //-1 for no patches
...
void loop()  //infinite working loop
{
	...
	if(button_state!=0) //A foot switch was pressed
	{
		...
		if ( button_state!=0 &&  abs(button_state) <12   )  //if key "Patch_up" / "Patch_down" or "Group_up" / "Group_down" is pressed
		{
			...
			//fetch the name of the pre-selected patch from "patches.h" (from PROGMEM)
...
{					  		
	...
	tft.fillRect(0, y, tft.width(), 2*17, ST7789_BLACK); // Clear area

	tft.setTextColor(color);
    
	if(pana.length()<100 && pana.length()>32)
	{		 
		String tmp = pana.substring(32, (pana.length() )%64);
		w=getStringWidth(tft,tmp);
		printAt(tft, tft.width()/2 - w/2 ,y+17, tmp); 
	}

	if(pana.length()>0)
	{
		w=getStringWidth(tft,pana.substring(0,32));
		printAt(tft,tft.width()/2 - w/2,y,pana.substring(0,32)); // Print first 13 chars of patch name as first row		
	}

	if(pana.length()==0)
	{
		printAt(tft,tft.width()/2-w/2, y,"None" ); // Print first 13 chars of patch name as first row		
	}
}
